...
double rumusC(double a, double b, int n, int k) {
	...
	return (a + (1-(fibo(n-k-1)/fibo(n-k))) * (b-a));
}

//rumus mencari elemen d
double rumusD(double a, double b, int n, int k) {
	
	return (a + (fibo(n-k-1)/fibo(n-k)) * (b-a));
}

//menghitung error toleransi
double errorToleransi(int n) {
...
int main() {
	...
	//mencari k, batas iterasi
	if (errorT != 0) {
		
		while (fibo(k) <= (b[0]-a[0])/errorT) {
		...
		while (fibo(k) <= (b[0]-a[0])/pangkat1(10, -4)) {
			
			fn = (b[0]-a[0])/pangkat1(10, -4);
			k++;
		}	
	}
	
	//mencari nilai c dan d
	c[0] = rumusC(a[0], b[0], k, 0);
	d[0] = rumusD(a[0], b[0], k, 0);	
	...
	for (iterasi=1; iterasi<=k; iterasi++) {
		...
		if (iterasi == 1) {
			...
			cout << setprecision(digit) << "\n c[" << iterasi-1 << "] = a[" << iterasi-1 << "] + (1 - (fibo(n-k-1) / fibo(n-k))) * (b[" << iterasi-1 << "] - a[" << iterasi-1 << "])" << endl;
			cout << setprecision(digit) << " c[" << iterasi-1 << "] = " << a[iterasi-1] << " + (1 - (" << fibo(k-iterasi-1-1) << " / " << fibo(k-iterasi-1) << ")) * (" << b[iterasi-1] << " - " << a[iterasi-1] << ")" <<  endl;			
			cout << setprecision(digit) << " c[" << iterasi-1 << "] = " << c[iterasi-1] << endl;
				
			cout << setprecision(digit) << "\n d[" << iterasi-1 << "] = a[" << iterasi-1 << "] + (fibo(n-k-1) / fibo(n-k)) * (b[" << iterasi-1 << "] - a[" << iterasi-1 << "])" << endl;
			cout << setprecision(digit) << " d[" << iterasi-1 << "] = " << a[iterasi-1] << " + (" << fibo(k-iterasi-1-1) << " / " << fibo(k-iterasi-1) << ") * (" << b[iterasi-1] << " - " << a[iterasi-1] << ")" <<  endl;
			...
			cout << setprecision(digit) << " d[" << iterasi-1 << "] = a[" << iterasi-1 << "] + (fibo(n-k-1) / fibo(n-k)) * (b[" << iterasi-1 << "] - a[" << iterasi-1 << "])" << endl;
			cout << setprecision(digit) << " d[" << iterasi-1 << "] = " << a[iterasi-1] << " + (" << fibo(k-iterasi-1-1) << " / " << fibo(k-iterasi-1) << ") * (" << b[iterasi-1] << " - " << a[iterasi-1] << ")" <<  endl;
			cout << setprecision(digit) << " d[" << iterasi-1 << "] = " << d[iterasi-1] << endl;
		}
		...
		//menampilkan jawaban untuk iterasi > 1 dengen mengecek kondisi fibonacci search
		if (f(c[iterasi-1]) <= f(d[iterasi-1])) {
			
			a[iterasi] = a[iterasi-1];
			b[iterasi] = d[iterasi-1];
			c[iterasi] = rumusC(a[iterasi], b[iterasi], k, iterasi);
			...
			cout << " c[" << iterasi << "] = " << a[iterasi] << " + (1 - (" << fibo(k-iterasi-1) << " / " << fibo(k-iterasi) << ")) * (" << b[iterasi] << " - " << a[iterasi] << ")" <<  endl;
			...
			d[iterasi] = rumusD(a[iterasi], b[iterasi], k, iterasi);
			...
			cout << " d[" << iterasi << "] = " << a[iterasi] << " + (" << fibo(k-iterasi-1) << " / " << fibo(k-iterasi) << ") * (" << b[iterasi] << " - " << a[iterasi] << ")" <<  endl;
		}
	}
